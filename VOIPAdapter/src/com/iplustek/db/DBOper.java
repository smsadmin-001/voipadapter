...
public class DBOper {
	...
	public DBOper() throws Exception{
	...
	private void updateOracleProcessStatus(Set<String> id_set,String status){
		...
		if(m_tract){
			logger.info("It takes "+timer.getPeriodTime()+" to update oracle, the data size is "+ id_set.size());
		}
	}
	...
	public void updateNosqlProcessStatus(Set<String> id_set,String status){
		...
		timer.setNowTime(new Date());
		if(m_tract){
			logger.info("it takes "+timer.getPeriodTime()+" to update nosql, the data size is "+ id_set.size());
		}
	}
	...
	public void updateNosqlProcessStatusOnce(String file_name,String status){
		int resu = 0;
		String sql = "update "+pl.getM_nosql_table_name()+" set "+pl.getM_nosql_process_status_name()+"="+status+" where "+pl.getM_nosql_id_name()+"= '"+file_name+"'";
		try {
			resu = nosql_executor.safeUpdate(sql);
		} catch (Exception e) {
			// TODO Auto-generated catch block
		...
		if(m_tract){
			logger.info("update process status to "+status+" where filename is "+ file_name +" and return is "+resu);
		}
	}
	
	private void updateNosqlProcessStatusWithoutBatch2(Set<String> id_set,String status){
		//int resu = 0;
		String sql;
		List<Object[]> values=new ArrayList<Object[]>();
		for(String id: id_set){
		...
		sql = "insert into T_TEMP(VAR) values(?)";
		try {
			nosql_executor.safeUpdate("truncate table T_TEMP");
			nosql_executor.safeInsertOrUpdate(sql, values);
		} catch (Exception e) {
				// TODO Auto-generated catch block
				logger.error(e.toString());;
		}
		sql = "update zrtp set "+pl.getM_nosql_process_status_name()+" = "+status+" where "+pl.getM_nosql_id_name()+" in (select VAR from T_TEMP)";
		
		try {
			nosql_executor.safeUpdate(sql);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			//e.printStackTrace();
			logger.error(e.toString());
	...
	private void updateNosqlProcessStatusInPstBatch(Set<String> id_set,String status){
	...
	public List<SimpleEngineResult> getSimpleEngineResult() throws Exception{
		...
		//sql = "select * from t_iv_voice_record_system where rownum < 1000";
		//System.out.println("get result sql "+sql);
	...
	public Set<String> getUndoneKeySet() throws Exception{
		...
		nosql_executor.closeMRes();
		nosql_executor.closeMStm();
		return id_set;
	}
	...
	public void insertOneVoiceRecord(VoiceRecord record) throws Exception{
		String pst_sql = "insert into " + pl.getM_oracle_table_name()+"(NID, SFILENAME, STARTTIME, NCODERATE, NCODEFORMAT) values(SEQ_ID.Nextval, ?, to_date(?,'YYYY-MM-DD HH24:MI:SS'),"+pl.getM_code_rate()+","+pl.getM_code_format()+")";
		List<Object[]> values = new ArrayList<Object[]>();
		values.add(new Object[]{record.getSFILENAME(),record.getSTARTTIME()});
		oracle_executor.safeInsertOrUpdate(pst_sql, values);
	}
	...
	public static void main(String[] args) throws Exception{
		...
		Connection con = dbo.nosql_executor.getBatchConnection();
		String pst_sql = "update zrtp set speech_process_status = 777 where id=?";
		PreparedStatement pst = con.prepareStatement(pst_sql);
		int index = 0;
		for(String s: id_set){
